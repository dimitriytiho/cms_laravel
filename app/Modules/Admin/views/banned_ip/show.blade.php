            ...
            <div class="col">
                <div class="table-responsive">
                    <table class="table table-striped">
                        ...
                        <tr>
                            <th class="font-weight-light" scope="row">{{ __("f.qty") }}</th>
                            <td>{{ $values->count }}</td>
                        </tr>
                        <tr>
                            <th class="font-weight-light" scope="row">{{ __("f.banned") }}</th>
                            <td>{{ $values->banned }}</td>
                        </tr>
                        <tr>
                    ...
                    <form action="{{ route("admin.$route.destroy", $values->id) }}" method="post" class="text-right mb-5 confirm-form">
                        @method('delete')
                        @csrf
                        <button type="submit" class="btn btn-outline-primary mt-3 btn-pulse">{{ __('s.clear') }}</button>
                    </form>
                </div>
            </div>
        </div>
