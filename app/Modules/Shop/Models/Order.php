...
class Order extends Model
{
     ...
     *
     *
     * Создаём 2 страницы доступные по Url: /error-payment (обычныя страница), /success-payment (пример ниже)
     * public function successPayment(Request $request)
        {
        // Номер заказа, который присвоил $orderId Сбербанк, передаёт Сбербанк Get параметром
        $orderIdSberbank = $request->query('orderId');

        // Проверям данные
        $check = Order::checkPaymentSberbank($orderIdSberbank);
        if (!$check) {

        // Если проверка не пройдена, то редирект на страницу ошибки
        return redirect()->route('error_payment');
        }

        $f = Str::snake(__FUNCTION__);
        Main::viewExists("{$this->viewPathModule}.{$this->c}_{$f}", __METHOD__);
        $this->setMeta('Успешная оплата');
        return view("{$this->viewPathModule}.{$this->c}_{$f}");
        }
     *
     *
                ...
                if ($statusSuccess && !empty($res['orderNumber']) && $res['orderNumber'] == $order->id) {
                    ...
                    if ($user) {
                        ...
                        try {
                            ...
                            // Отправить письмо
                            Mail::to($user->email)
                                ->send(new SendMail($title, $body, null, 'service'));
