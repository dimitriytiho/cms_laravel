...
{
    // Настройки
    protected $tpl; // Передать название шаблона из папки tpl
    ...
    // Настройки запроса
    protected $table = 'menu'; // Необходимо, чтобы была модель, передаваемой таблицы
    protected $where = []; // 'id', '7' К примеру где id = 7 или множественно [['id', '7'], ['accept', '1'],]
    ...
    public function __construct($options = [])
    {
        ...
        $this->tpl = is_file(MENU . "{$this->tpl}.php") ? MENU . "{$this->tpl}.php" : __DIR__ . '/tpl/default.php';
        $this->name = class_basename(__CLASS__);
        //self::init();
        $this->run();
    }
    ...
    protected function run()
    {
        ...
        $this->cacheName = $this->cacheName ?: "{$this->name}_{$this->tpl}";
        ...
        if (!$this->menuHtml) {
            ...
            if (!$this->data) {
                ...
                if ($this->sql) {
                    ...
                    if ($this->where) {
                        $this->data = DB::table($this->table)->where($this->where)->orderBy($this->orderBy, $this->sort)->get()->keyBy('id');
